            ...
            <div className="flex justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    ...
                    <div className="relative user-menu ml-2">
                        ...
                        >
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center shadow-lg border-2 border-card">
                                <User size={18} className="text-primary-foreground" />
                            </div>
                            <div className="hidden lg:block text-left">
                                <p className="text-sm font-black text-foreground leading-none">{user?.name || 'User'}</p>
                                <p className="text-[10px] text-muted-foreground mt-1 font-bold uppercase tracking-tighter">{user?.role || 'Member'}</p>
                            </div>
                        </button>
                        ...
                        {isUserMenuOpen && (
                            <div className="absolute right-0 mt-3 w-60 bg-card border border-border rounded-2xl shadow-2xl p-2 z-50 animate-in fade-in slide-in-from-top-2">
                                <div className="px-4 py-3 border-b border-border mb-2">
                                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Account Info</p>
                                    <p className="text-sm font-black text-foreground truncate">{user?.email}</p>
                                </div>
                                <Link href="/setting" onClick={() => setIsUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-foreground hover:bg-primary/10 rounded-xl transition-all">
